...
#define EXPECT_FAIL(TP, HOST) do {                                      \
  ...
  /* Listen on ipv6 loopback, if it fails skip ipv6 tests.
     ...
     NOTE: Don't the unspecified address "::" here - ipv6-disabled platforms may
     allow listening on "::" without complaining, even though it effectively
     listens on zero interfaces. However they won't have a local ipv6 loopback
     configured, so "::1" will force an error.
  */
  TEST_PROACTORS_DRAIN(tps);
