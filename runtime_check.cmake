...
# Any test that needs runtime checks should use TEST_PREFIX and TEST_ENV.
...
# e.g. -DTEST_PREFIX=rr or -DTEST_PREFIX="valgrind --tool=massif"

set(TEST_PREFIX "" CACHE STRING "Prefix for test executable command line")
set(TEST_PREFIX_EXT "" CACHE STRING "Prefix for interpreter tests (e.g. python, ruby) that load proton as an extension")
set(TEST_ENV "" CACHE STRING "Extra environment for tests: name1=value1;name2=value2")
mark_as_advanced(TEST_PREFIX TEST_PREFIX_EXT TEST_ENV)
...
set(VALGRIND_COMMON_ARGS "--error-exitcode=42 --quiet --suppressions=${VALGRIND_SUPPRESSIONS} --gen-suppressions=all")
...
if(NOT CMAKE_BUILD_TYPE MATCHES "Coverage") # No checks in coverage build
  if (HAS_SANITIZERS)
    set(RUNTIME_CHECK_DEFAULT "asan")
  elseif(VALGRIND_EXECUTABLE)
...
if(RUNTIME_CHECK STREQUAL "memcheck")
  ...
  set(TEST_PREFIX "${VALGRIND_EXECUTABLE} --tool=memcheck --leak-check=full ${VALGRIND_COMMON_ARGS}")
  # FIXME aconway 2018-09-06: NO TEST_PREFIX_EXT, need --trace-children + many suppressions

elseif(RUNTIME_CHECK STREQUAL "helgrind")
  assert_has_valgrind()
  message(STATUS "Runtime race checker: valgrind helgrind")
  set(TEST_PREFIX "${VALGRIND_EXECUTABLE} --tool=helgrind ${VALGRIND_COMMON_ARGS}")
  # FIXME aconway 2018-09-06: NO TEST_PREFIX_EXT, need --trace-children + many suppressions
...
elseif(RUNTIME_CHECK STREQUAL "asan")
  ...
  set(TEST_PREFIX_EXT "${CMAKE_SOURCE_DIR}/tests/preload_asan.sh $<TARGET_FILE:qpid-proton-core>")
...
if(TEST_PREFIX)
  # Add TEST_PREFIX to TEST_ENV so test runner scripts can use it.
  list(APPEND TEST_ENV "TEST_PREFIX=${TEST_PREFIX}")
  # Make a CMake-list form of TEST_PREFIX for add_test() commands
  separate_arguments(TEST_PREFIX_CMD UNIX_COMMAND "${TEST_PREFIX}")
endif()
separate_arguments(TEST_PREFIX_EXT_CMD UNIX_COMMAND "${TEST_PREFIX_EXT}")
