...
void test_all() {
    ...
    ASSERT_EQUAL("foobar.org",  *opts.virtual_host());
    ASSERT_EQUAL(true,  *opts.sasl_enabled());
    ASSERT_EQUAL("fred", *opts.user());
    ASSERT_EQUAL("a b c", *opts.sasl_allowed_mechs());
    ASSERT(*opts.sasl_allow_insecure_mechs());
    // FIXME aconway 2018-09-21: need to set up real credentials to
    // test SSL settings, they are applied immediately and there will
    // be an exception if they are not valid.
}

void test_defaults() {
    connection_options opts;
    ASSERT_EQUAL("localhost:amqps", configure(opts, "{}"));
    ASSERT_EQUAL("localhost",  *opts.virtual_host());
    ASSERT_EQUAL(true,  *opts.sasl_enabled());
    ASSERT(!opts.user());
    ASSERT(!opts.sasl_allowed_mechs());
    ASSERT(!opts.sasl_allow_insecure_mechs());
    ASSERT(!opts.ssl_client_options());
}
